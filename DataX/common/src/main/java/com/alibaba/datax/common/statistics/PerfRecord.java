...
public class PerfRecord implements Comparable<PerfRecord> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(PerfRecord.class);
    ...
    public void start() {
        logger.info("perfrecord is enabled? {}", PerfTrace.getInstance().isEnable());
        if (PerfTrace.getInstance().isEnable()) {
            ...
            perf.info("perfrecord.start,{}",toString());
        }
    }
